...
Para (参数) - 继承自 DynamicAttribute，不是 Onto 子类
```

**关键说明**:
- `Onto` 是核心基类，本身包含 `paras`（参数列表）、`rules`（规则列表）、`attrParas`（属性参数）等核心字段
- `ModuleBase` 继承 `Onto`，额外包含 `atomicParts` 和 `partCategorys` 来组织部件结构
- `Para` 继承 `DynamicAttribute`，用于表示可配置的参数选项
