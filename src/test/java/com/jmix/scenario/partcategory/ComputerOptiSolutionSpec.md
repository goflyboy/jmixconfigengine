...
##packageName
```java 
com.jmix.scenario.partcategory
```

##modelName
```java 
ComputerOptiSolution
```
##userVariableModel
```java
...
rule2://固态硬盘优先匹配高速率容量，用机械硬盘增配低速率容量
 solidCapacityWeight = solidDrive1.isSelected *  solidDrive1. capacityWeight + solidDrive2.isSelected *  solidDrive2. capacityWeight + solidDrive3.isSelected *  solidDrive3. capacityWeight 
...
##userTestCaseSpec
```java
 需要包含用例： partCatagory = "drive"  constraintReq1 = "sum.Quantity ==2 where Speed=540";  转化结果为：
