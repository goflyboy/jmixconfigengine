...
| 规则场景覆盖 | 以 `docbackup/Rule-Scenarios-And-SDK-Summary.md` 的规则场景矩阵为覆盖基线；当前 RuleTrans 只验证了最小 PartCategory 数量约束和产品级跨分类互斥，其他规则类型需要补场景测试 |
| SDK 上下文隔离 | 生成 Java 片段时必须按 `POST` 与非 `POST` 分开构建 SDK 上下文；两者可共享领域事实投影，但不可共享南向 API 提示词 |
...
| 前期未明确 SDK 包装边界 | 提升 Java 片段准确率需要把南向 API 分层暴露给生成器 | 增加 `POST` 与非 `POST` SDK 上下文隔离要求，禁止两类 API 混用 |
...
1. 规则类型覆盖不足：当前 RuleTrans 测试主要覆盖 `CPU at most one` 和 `4-core CPU cannot use 5400 drive`，只能证明最小 PartCategory 数量约束和产品级跨分类互斥可走通，不能证明参数兼容、计算、隐藏、优先级、多实例、POST、结构化组合等规则类型可生成。
2. 中文输入不足：现有 RuleTrans 测试与 Prompt 样例主要是英文；实际业务输入会出现“最多一块”“四核”“转速 5400 转”“固态盘”“硬盘”等中文描述、中文单位和业务别名。
3. SDK 上下文混合风险：非 `POST` 规则基于约束模型构建 CP 变量、表达式和约束；`POST` 规则基于求解后的实例 view 读写参数和实例属性。两者依赖完全不同，生成 Java 片段时必须分别构建上下文、模板和可用 API 列表。
...
| 上下文构建 | `RuleContextFactory`、`PartCategoryRuleContext`、`ProductRuleContext` | 已支持从 `Module` 和注解类构建上下文 |
| Prompt 投影 | `RulePromptProjector`、`PartCategoryPromptView`、`ProductPromptView` | 已覆盖参数、属性参数、动态属性、部件、子分类 |
| 产品级分类识别 | `CategoryIdentifier`、`product_stage1_prompt.jtl` | 已支持 LLM 输出分类 code 并校验存在性 |
| 非 POST 代码生成 | `part_category_prompt.jtl`、`product_stage2_prompt.jtl` | 已偏向 `model()`、`sum4Selected`、`sum4Quantity`、`inCompatible` 等约束模型 API |
| 代码片段后处理 | `RuleSnippetPostProcessor` | 已拒绝 package/import/class，清理 `@CodeRuleAnno(code=...)` |
...
本节以 `docbackup/Rule-Scenarios-And-SDK-Summary.md` 的矩阵为基线。这里的“规则引擎状态”表示现有南向 SDK/执行器是否已有代表测试；“RuleTrans 状态”表示自然语言到 Java 片段模块是否已有对应生成能力和测试。
...
| 产品级跨分类部件互斥 | 已覆盖：`MultiPCTest`、`OptionalPartCategoryWhitelistGuardTest` | 已基础支持：`CategoryIdentifierTest`、`RuleTransModuleTest` | 补中文分类别名、中文单位、validate/recommend 双路径 |
...
| 产品级参数优先类样例 | 文档 TODO 清单标记缺失 | RuleTrans 未覆盖 | 补参数目标函数/偏好测试 |
...
2. 汇总/选择/优先级规则：跨分类数量/属性汇总、多实例 `SumSum`、exactly-one/at-most-one、优先级目标函数、产品级参数优先。
3. POST 规则：产品级参数写回、部件分类级参数写回、部件级实例写回、动态属性读写。POST 必须使用实例 view SDK，不得复用非 POST Prompt。
...
RuleTrans 不能只按 `ProductRuleContext` / `PartCategoryRuleContext` 选择模板。生成 Java 片段前必须先判断规则运行环境和规则族：
...
- 产品级非 `POST` 与部件分类级非 `POST` 可以共享 `ConstraintSdkPromptView` 字段结构，但 target categories、可见参数和可见部件范围必须按 scope 裁剪。
- 产品级 `POST` 与部件分类级 `POST` 可以共享 `PostSdkPromptView` 字段结构，但必须区分 `parameter(code)` 和 `partCategory(code).parameter(code)` 的写回目标。
- `@CodeRuleAnno(calcStage = CalcStage.POST)` 必须由 POST 路径显式生成；非 POST 路径不得写 `calcStage = CalcStage.POST`。
- `RuleSnippetPostProcessor` 应增加 SDK profile 静态校验：非 POST 片段出现 `currentInst` / `setValue` 判失败；POST 片段出现 `model()` / CP 约束 API 判失败。
...
├── post/
│   ├── ProductPostCalcRuleTransScenarioTest.java
...
- `Scenario` 测试定位为整体拉通：自然语言/结构化输入 -> 分类识别 -> SDK 上下文 -> Java 片段/转换器 -> 编译 -> validate/recommend/POST 执行。
...
POST 片段必须显式声明 `calcStage = CalcStage.POST` 并只使用实例 view：

```java
@CodeRuleAnno(calcStage = CalcStage.POST, normalNaturalCode = "把硬盘总容量写回产品参数")
public void postDriveCapacity() {
...
- 在非 `POST` 片段中使用实例 view 写回 API。
- 在 `POST` 片段中使用 `model()`、CP 变量或约束 API。
...
- 明确要求 `@CodeRuleAnno(calcStage = CalcStage.POST, normalNaturalCode = "...")`。
...
- `Scenario` 测试走完整链路：输入 -> 分类/场景识别 -> SDK profile -> 片段或转换器 -> 编译 -> 执行。
...
- 非 `POST` 片段只允许 `ConstraintSdkContext` API。
- `POST` 片段只允许 `PostSdkContext` API。
...
```java
@Test
public void testRejectConstraintSnippetUsingPostApi() {
    String snippet = """
            @CodeRuleAnno(normalNaturalCode = "CPU at most one")
            public void badRule() {
    ...
    assertThrows(RuleTransException.class,
            () -> postProcessor.process(snippet, SdkProfile.CONSTRAINT));
}
```
...
- POST 生成样例包含 `@CodeRuleAnno(calcStage = CalcStage.POST, ...)`。
...
### AC-015：测试确定性与真实 LLM 隔离

规则场景回归测试默认使用 fake `LLMInvoker` 或结构化输入，确保 `mvn test` 不依赖真实大模型和网络。真实 LLM 端到端测试可以保留，但必须：

- 单独命名或打标签，避免普通回归被 API key / 网络状态影响。
- 不作为规则类型覆盖的唯一证据。
- 失败时能输出 prompt、response、profile 和场景编号，便于复现。
...
| 2 | 实现 `RuleContext`、`PartCategoryRuleContext`、`ProductRuleContext`、`RuleContextFactory` | P0 | 已实现 |
...
| 5 | 实现 `CategoryIdentifier`，完成产品级 Stage1 分类识别与校验 | P0 | 已实现 |
| 6 | 实现 `RuleSnippetGenerator`，通过构造注入复用 `LLMInvoker` | P0 | 已实现 |
| 7 | 实现 `RuleSnippetPostProcessor`，提取代码块并校验只包含 rule 方法片段 | P0 | 已实现，已清理 `@CodeRuleAnno(code=...)` |
...
| 11 | 实现 `RuleTestCaseGenerator` 与 `RuleTransTestCaseSet` JSON schema | P1 | 已实现基础版本 |
| 12 | 实现 `RuleSnippetAssembler.assembleExecutableTest(...)`，生成 `ModuleScenarioTestBase` 风格测试 | P1 | 已实现基础版本 |
| 13 | 实现 `TestExecutionProcessor` 和结构化 JUnit 失败结果 | P1 | 已实现 |
| 14 | 实现测试失败纠错循环 | P1 | 已实现基础版本 |
| 15 | 端到端集成测试：自然语言 -> 代码片段 -> 编译 -> 测试通过 | P1 | 已有最小样例，覆盖不足 |
...
| A3 | 拆分非 POST 与 POST Prompt，新增 `post_product_prompt.jtl`、`post_part_category_prompt.jtl`、`post_part_prompt.jtl` | P0 | 待实现 |
| A4 | 增强 `RuleSnippetPostProcessor`，按 `SdkProfile` 静态拒绝跨 profile API | P0 | 待实现 |
| A5 | 建立 `src/test/java/com/jmix/ruletrans/rulescenario` 子包，按规则类型拆分场景测试 | P0 | 待实现 |
| A6 | 补中文分类识别与中文自然语言场景，包括中文单位、中文别名、中英混合字段 | P0 | 待实现 |
| A7 | 补参数兼容、计算、隐藏、汇总、选择、优先级、多实例规则场景 | P1 | 待实现 |
| A8 | 补 POST 产品参数写回、分类参数写回、部件实例写回场景 | P1 | 待实现 |
| A9 | 为结构化二元/三元组合、结构化 POST 设计确定性转换器 | P1 | 待实现 |
| A10 | 为规则场景回归测试引入 fake `LLMInvoker` 或结构化 fixture，真实 LLM 测试独立隔离 | P1 | 待实现 |
| A11 | 当分类识别、过滤表达式、上下文投影等基础能力不足时，新增极简 API 测试 | P1 | 持续执行 |
...
风险：模型把 `model()`、CP 变量、`parameter().setValue(...)`、`partCategorySum(...)` 混在同一个片段里，导致编译通过但运行阶段语义错误，或编译阶段直接失败。
...
- 规则场景覆盖默认使用 fake `LLMInvoker` 或结构化 fixture。
- 真实 LLM 端到端测试单独命名或打标签。
- 真实 LLM 测试失败时输出 prompt、response、`SdkProfile` 和场景编号。
...
本轮调研结论不阻塞后续实现。以下事项只影响排期顺序：

1. 是否优先实现 `POST` SDK profile 隔离，再补非 POST 复杂规则场景？建议优先实现，因为当前 Prompt 已明显偏向非 POST。
2. 真实 LLM 端到端测试是否从普通回归中拆出单独 profile/tag？建议拆出，规则场景回归使用 fake `LLMInvoker`。
3. 结构化规则转换器是否先覆盖二元/三元组合，再覆盖结构化 POST？建议先覆盖二元/三元组合，因为现有规则引擎测试证据最充分。
