...
| 新部件约束路径 | `InferParasReq.partConstraintReqs` 非空，或直接处理 `InferPartCategoryReq` | `ModuleConstraintExecutorImpl.processProduct()` → `ModuleBaseConstraintExecutorImpl` | 未接入松弛诊断 |
...
- `processProduct()` 路径即使开启 `debugByRelaxVar`，也可能只得到普通 `SolverResult`。
