...
6. **封装复杂输入**：如果测试需要反复创建请求对象、ModuleInst、PartInst 或校验请求，优先放到测试基类 helper，具体测试方法保持 1-3 行业务断言
7. **拆分测试类**：当新增场景会让原测试类过大时，新建独立测试类覆盖补充场景，例如主类测白名单，补充类测黑名单和边界
...
当 RFC 涉及结构化规则、组合规则、注解生成规则、Schema 展开规则时，必须按下表验收：

| 验收项 | 必须覆盖的测试 |
|--------|----------------|
| 产品侧声明 | 测试模块使用注解/Schema/配置声明规则，验证能生成 Module 基础数据 |
| 生成路径 | 通过 `inferParas`、`inferRecommendModule` 或同类高层入口验证规则参与求解 |
| 校验路径 | 通过 `validData(...)`、`validateData(...)` 或同类 validator helper 验证给定 `ModuleInst` |
| WHITE 组合 | 命中允许组合通过，未命中默认失败 |
| BLACK 组合 | 命中禁止组合失败，未命中默认通过 |
| 元数覆盖 | 二元和三元组合分别覆盖 |
| 边界覆盖 | 表达式无匹配、重复组合去重、运行时过滤、子规则不独立执行 |
| 诊断覆盖 | 校验失败时断言父规则 code 或等价诊断信息 |

测试代码要求：

- 具体测试方法优先保持 1-3 行，复杂构造封装到测试基类或 helper。
- 不在测试方法里直接拼装大量 `InferParasReq`、`ModuleValidateReq`、`PartConstraintReq`；优先复用 `inferRecommendModule(...)`、`validData(...)`、`validateData(...)` 等接口。
- 规则表达式优先使用业务友好的紧凑表达，避免让测试样例复制内部实现字段结构。
- 当一个测试类已经覆盖主路径时，补充枚举值或边界场景应新建独立测试类，避免主测试类变成混杂场景。
