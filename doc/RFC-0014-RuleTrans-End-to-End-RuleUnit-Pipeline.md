...
替代当前偏混合的 `RuleTransResult`，作为端到端主入口的唯一返回结构。旧的 `translateWithRetry(...)` 不再保留兼容签名，也不再通过增强 `RuleTransResult` 继续承载新流程。
...
5. 只在内存中转发业务 JSON 用例，不负责持久化文件。

`RuleTransEngine` 保留为底层翻译协作者，继续提供生成 Java 方法体所需的内部能力；端到端调用方统一改用 `RuleTransPipeline.execute(...)`。旧的 `translateWithRetry(...)` 直接删除，调用方不再通过同名方法或兼容 facade 进入新流程。
...
| `RuleTransEngine.translateWithRetry(...)` | 直接删除；所有调用方改为 `RuleTransPipeline.execute(...)` |
...
| `TestExecutionProcessor` | 从 `src/main/java` 直接删除，不移动到 `src/test/java` |
...
| 类型 | 处理建议 |
| --- | --- |
| 历史 scenario 测试中的 `RuleScenarioHarnessSupport` | P0 必须改为业务 JSON + RuleUnit 执行器路径，不再包装 `TestExecutionProcessor` |
| `RuleTransTestCase` / `RuleTransTestCaseSet` | 不再作为主流程测试事实模型；若仅有历史断言临时需要，可在 `src/test` 中短期保留数据对象，但不得依赖 JUnit 生成执行路径 |
...
P0 明确不把业务 JSON 写入 `src/test/resources/rule-unit-cases` 或其他源码目录。RuleTrans 在这里是自然语言翻译、编译、用例生成和 RuleUnit 验证的转发/编排服务，业务 JSON 只作为本次执行的内存事实来源进入 `RuleTransPipelineResult`。

后续规则管理模块需要记录规则、用例、审核状态和版本时，应在规则管理边界内设计持久化模型，而不是让 RuleTrans pipeline 顺手落盘。

因此本 RFC 不定义 `persistBusinessCases`、`businessCaseOutputRoot`、覆盖策略或文件命名策略。
...
| 当前类 | 建议 |
| --- | --- |
| `RuleTransResult` | 不继续承载端到端验证结果；新流程直接返回 `RuleTransPipelineResult` |
...
| `TestExecutionProcessor` | 直接删除，不移动到 test-scope，不再被 source 或测试迁移路径依赖 |
...
| P0-4 | 删除 `RuleTransEngine.translateWithRetry(...)`，调用方统一迁移到 `RuleTransPipeline.execute(...)`，并移除 `TestExecutionProcessor` 构造器依赖 | P0 |
...
| P0-7 | 直接删除 `TestExecutionProcessor`，同步删除或重写 `TestExecutionProcessorTest` | P0 |
| P0-8 | 更新 `src/test` 中的 RuleTrans 回归测试和 scenario harness，使用 fake LLM + RuleUnit 执行器报告断言，不再断言 JUnit Launcher 路径 | P0 |
| P0-9 | 增加静态约束测试，确保 `src/main/java/com/jmix/ruletrans` 不依赖 JUnit Launcher | P0 |
| P1-1 | 增强“代码错 / 用例错”自动判别 prompt | P1 |
| P1-2 | 与后续规则管理模块对接业务 JSON 记录能力；不在 RuleTrans pipeline 内直接落盘 | P1 |
...
### AC-004: 旧 `translateWithRetry(...)` API 不再存在

执行：

```powershell
rg "translateWithRetry" src/main/java src/test/java
```

结果应为空。端到端调用方只能通过 `RuleTransPipeline.execute(...)` 进入新流程。

### AC-005: `src/test` 不再调用旧执行处理器

执行：

```powershell
rg "TestExecutionProcessor|assembleExecutableTest" src/test/java
```

结果应为空。原本依赖这些入口的 `RuleScenarioHarnessSupport`、`TestExecutionProcessorTest` 或相关 scenario 测试，必须删除或改为调用新的 RuleUnit 单元测试执行器。
...
### AC-007: 业务 JSON schema 错误触发用例重生成
...
### AC-010: RuleTrans pipeline 不写业务 JSON 文件

端到端执行成功后，`RuleTransPipelineResult.businessCaseSet` 包含本次执行的业务 JSON 用例，但 RuleTrans pipeline 不创建或更新 `src/test/resources/rule-unit-cases` 下的文件。业务 JSON 持久化由后续规则管理模块设计。
...
| `RuleScenarioHarnessSupport` 相关测试 | 改为组装规则类、生成业务 JSON，并调用新的 RuleUnit 单元测试执行器 |

不再保留或需迁移：

| 测试 | 处理 |
| --- | --- |
| `TestExecutionProcessorTest` | 直接删除；若有仍有价值的断言，迁移到 `RuleTransPipelineTest` 或 RuleUnit 执行器测试 |
...
`RuleSnippetAssembler` 的旧 JUnit 生成路径在一些 scenario 测试中仍被使用。P0 必须把这些测试改成 RuleUnit 路径，不能通过把 `TestExecutionProcessor` 移动到 `src/test/java` 来延续旧执行链路。若某个历史测试迁移成本过高，应先删除或标记为待重建测试，并在实现记录中说明覆盖缺口。
...
1. `TestExecutionProcessor` 直接删除，不移动到 `src/test/java` 作为历史工具。删除时必须同步检查 `src/test`，把依赖它的测试改为新的 RuleUnit 单元测试执行器，或删除无保留价值的旧路径测试。

2. RuleTrans pipeline 暂不持久化业务 JSON。后续规则管理能力负责规则、用例和版本记录；本流程只做翻译、编译、业务用例生成和 RuleUnit 验证的内存转发。

3. 不保留旧的 `translateWithRetry(...)` 方法签名，也不做仅标记废弃的兼容期。端到端主入口直接切换为 `RuleTransPipeline.execute(...)`，调用方一次性迁移。
